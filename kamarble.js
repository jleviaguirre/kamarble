const SPREADSHEET_ID = "1KtSb4Nb313vxJ4KgZxNoFqjAwKVh1jGxcXsLcbfxfVo";
...
render = data => {
  let template = _.template(document.getElementById("tpl-data").innerHTML);
  document.getElementById("spreadsheet-data").innerHTML = template({
    data: data
  });
  
  new WholePageSlider({
    colors: ['black', 'deepskyblue', 'orange', 'lightgray', 'lightblue',
...
axios
  ...
  .then(r => {
    let sheets = _.map(_.get(r, "data.feed.entry"), "content.$t"),
      counter;

    counter = _.size(sheets);

    _.each(sheets, (sheet, i) => {
      axios
        .get(
          `https://spreadsheets.google.com/feeds/list/${SPREADSHEET_ID}/${i +
            1}/public/values?alt=json`
        )
        .then(r => {
          let entries = _.get(r, "data.feed.entry"),
            prefix = "gsx$",
            items = [],
            c;
